...
request.addEventListener("input", e => {
  ...
  //Объявление переменных
  const inputFieldBlock = document.querySelector(".header .search-form");
  ...
  if (req.length === 0) {
    ...
    if (searchBlock.childElementCount >= 3) {
      while (searchBlock.childElementCount < 3) {
        searchBlock.removeChild(searchBlock.lastChild);
      }
    }
    return searchBlock;
...
function createResultBlock(desiredItem) {
  //Массив содержит только те элементы, которые удовлетворяют условиям
  let arrFoundItems = productsData.filter(item => {
    return (item.name.toLowerCase().indexOf(desiredItem) !== -1 ||
                item.brand.toLowerCase().indexOf(desiredItem) !== -1);
  });

  //Копия массива arrFoundItems, содержит ограниченное количество найденных элементов
  const arrFirstElements = [].concat(arrFoundItems);
  ...
  if (arrFoundItems.length !== 0) {
    ...
    if (searchBlock.childElementCount < 3) {
      ...
      let resultTitle = document.createElement("h2");//заголовок "Лучшие совпадения"
      let ul = document.createElement("ul");//список, куда будут добавляться найденные элементы
      let ref = document.createElement("a");//ссылка на каталог, содержащий все результаты запроса

      //Добавление классов к элементам
      section.classList.add("result");
      resultTitle.classList.add("catalog");
      ul.classList.add("result__list-output", "list-output");
      ref.classList.add("link", "link--color__grey");
      ...
      arrFirstElements.forEach(item => {
        ...
        let innerRef = document.createElement("a");//ссылка на сам товар
        let span = document.createElement("span");//цена товара
    ...
    if (searchBlock.childElementCount < 3) {
      let parNotFound = document.createElement("p");
      parNotFound.textContent = "По вашему запросу ничего не найдено";
      parNotFound.classList.add("search-block__par", "par", "par--align-content");
      searchBlock.appendChild(parNotFound);
    }
  }
  console.log(arrFoundItems, arrFirstElements);
}
*/